...
def load_benchmark_results(results_dir: str) -> Dict[str, List[Dict[str, Any]]]:
    """Load benchmark result files: latest epoch per storage type only."""
...
def generate_markdown_report(results: Dict[str, List[Dict[str, Any]]], output_file: str):
    ...
    for storage_type in ["redis", "postgres", "sqlite"]:
        ...
        report.append(
            "| Scenario | Throughput (ops/sec) | P50 Latency (ms) | P95 Latency (ms) | P99 Latency (ms) | Errors |"
        )
        report.append(
            "|----------|---------------------|------------------|------------------|------------------|--------|"
        )
        ...
        for result in results[storage_type]:
            ...
            throughput = float(result.get("throughput_ops_per_sec", 0) or 0)
            ...
            report.append(
                f"| {scenario} | {throughput:.2f} | {p50:.2f} | {p95:.2f} | {p99:.2f} | {errors} |"
            )
    ...
    report.append(
        "- **Redis**: Best for high-throughput write workloads with low latency requirements\n"
    )
    report.append(
        "- **PostgreSQL**: Provides ACID guarantees but may have lower write throughput\n"
    )
    report.append(
        "- **SQLite**: Good for single-instance deployments but limited concurrency\n"
    )
