...
func (b *Benchmarker) newCoordGen(seed int64) CoordinateGenerator {
	...
	return NewZipfianGenerator(b.canvasWidth, b.canvasHeight, seed, b.config.ZipfianSkew)
}

func (b *Benchmarker) syntheticShardID(x, y uint) string {
	if b.config.Shards <= 1 {
		return "all"
	}
	shard := int((y*b.canvasWidth+x)%uint(b.config.Shards)) + 1
...
func (b *Benchmarker) generatePixelData(_, _ uint) []byte {
	pixel := &domain.RedisPixel{
		UserId:    fmt.Sprintf("bench_%d", secureIntn(1000)),
		Faculty:   fmt.Sprintf("faculty_%d", secureIntn(5)),
		Color:     []uint{uint(secureIntn(256)), uint(secureIntn(256)), uint(secureIntn(256))},
